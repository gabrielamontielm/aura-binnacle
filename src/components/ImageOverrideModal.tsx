      ...
      {isOpen && (
        <div className="fixed inset-0 bg-artistic-ink/90 backdrop-blur-md z-[200] flex items-center justify-center p-6">
          ...
          >
            ...
            {subtitle && (
              <p className="text-xs uppercase font-bold tracking-widest text-artistic-ink/40 mb-8">
                {subtitle}
              </p>
            )}
            <p className="text-xs uppercase font-bold tracking-widest text-artistic-ink/40 mb-8">
            ...
            <div className="space-y-6">
              {searchQuery && (
                <div>
                  <span className="text-[11px] uppercase font-bold tracking-[0.2em] mb-3 block">Step 1: Find Reference</span>
                  <a 
                  ...
                  >
                    ...
                    <span className="text-[11px] font-bold uppercase tracking-widest text-artistic-ink group-hover:text-white">Search Google Images</span>
                    <ExternalLink className="w-3 h-3 opacity-20 group-hover:opacity-60 transition-all text-artistic-ink group-hover:text-white" />
                  </a>
                  <p className="text-[11px] opacity-40 mt-2 text-center">Open search, right-click an image, and "Copy Image Address"</p>
                </div>
              )}

              <div>
                <span className="text-[11px] uppercase font-bold tracking-[0.2em] mb-3 block">{searchQuery ? 'Step 2: Social Mirroring' : 'Option 1: Social Mirroring'}</span>
              ...
              <div>
                <span className="text-[11px] uppercase font-bold tracking-[0.2em] mb-3 block">Option 2: Physical Deposit</span>
                <input 
                  type="file" 
                  ref={fileInputRef}
                ...
                >
                  ...
                  <span className="text-[11px] font-bold uppercase tracking-widest text-artistic-ink/40 text-artistic-ink">Select File</span>
                </button>
              </div>

              <button 
                onClick={onClose}
                className="w-full text-center text-[11px] font-bold uppercase tracking-widest py-2 opacity-40 hover:opacity-100 hover:text-red-500 transition-all mt-4"
              >
                Abort Operation
              </button>
            </div>
